...
class ConditioningAugmention(nn.Module):
    def __init__(self, input_dim, emb_dim, device):
        ...
        self.layer = nn.Sequential(
            nn.Linear(in_features=self.input_dim, out_features=self.emb_dim, bias=False), 
            nn.ReLU()
        ).to(device)
    ...
    def forward(self, x):
        ...
        condition = reparametrize(mu, log_sigma)
        ################# Problem 2-(a). #################
...
class ImageExtractor(nn.Module):
    def __init__(self, in_chans):
        ...
        print(f"ImageExtractor - self.in_chans: {self.in_chans}, self.out_chans: {self.out_chans}")

        self.image_net = nn.Linear(
            nn.ConvTranspose2d(in_channesl=self.in_chans, out_channels=self.out_chans), 
...
class Generator_type_1(nn.Module):
    ...
    def forward(self, condition, noise):
        ...
        '''
        print(condition.shape, noise.shape)

        out = self._upsample_network(self._mapping_network())(torch.cat(condition, noise))
        out_image = self._image_net()(out)
        ################# Problem 2-(c). #################
...
class Discriminator(nn.Module):
    ...
    def forward(self,
        ...
        if condition is None:
            out = self._uncond_discriminator()
            align_out = None

        else:
            out = self._cond_discriminator()
            align_out = self._align_cond_discriminator
